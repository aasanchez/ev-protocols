...
happens, the server should add the status code [3001](/ocpi/05-status-codes/05-status-codes.md#3xxx-server-errors) in
the response to the POST from the client.
...
with the status code [3003](/ocpi/05-status-codes/05-status-codes.md#3xxx-server-errors).
